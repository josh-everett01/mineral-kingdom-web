...
export function extractExpirationEpochSeconds(payload: JwtPayload): number | null {
...
export function getAccessTokenExpiresAtEpochSeconds(token: string): number | null {
  const payload = decodeJwt(token)
  if (!payload) return null
  return extractExpirationEpochSeconds(payload)
}

export function isJwtExpired(token: string, skewSeconds = 30): boolean {
  const exp = getAccessTokenExpiresAtEpochSeconds(token)
  if (!exp) return true
