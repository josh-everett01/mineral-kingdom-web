...
async function forwardOnce(invoiceId: string, accessToken: string) {
  const headers = new Headers()
  headers.set("accept", "application/json")
  headers.set("authorization", `Bearer ${accessToken}`)

  return fetch(`${API_BASE_URL}/api/shipping-invoices/${encodeURIComponent(invoiceId)}/payments/capture`, {
    method: "POST",
    headers,
    cache: "no-store",
  })
...
export async function POST(_req: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    upstream = await forwardOnce(invoiceId, access)
  } catch (e) {
    const err = toProxyError(
      ...
      "Shipping payment capture service unavailable",
    )
